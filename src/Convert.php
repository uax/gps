...
class Convert
{
    ...
    //WGS-84 to GCJ-02
    public function gcjEncrypt($wgsLat, $wgsLon)
    {
    ...
    //GCJ-02 to WGS-84
    public function gcjDecrypt($gcjLat, $gcjLon)
    {
    ...
    public function gcjDecryptExact($gcjLat, $gcjLon)
    {
        $initDelta = 0.01;
        ...
        while (true) {
            $wgsLat = ($mLat + $pLat) / 2;
            $wgsLon = ($mLon + $pLon) / 2;
            $tmp = $this->gcjEncrypt($wgsLat, $wgsLon);
            $dLat = $tmp['lat'] - $gcjLat;
            $dLon = $tmp['lon'] - $gcjLon;
    ...
    //GCJ-02 to BD-09
    public function bdEncrypt($gcjLat, $gcjLon)
    {
        $x = $gcjLon;
        $y = $gcjLat;
    ...
    //BD-09 to GCJ-02
    public function bdDecrypt($bdLat, $bdLon)
    {
        $x = $bdLon - 0.0065;
        $y = $bdLat - 0.006;
        $z = sqrt($x * $x + $y * $y) - 0.00002 * sin($y * $this->x_pi);
    ...
    public function mercatorEncrypt($wgsLat, $wgsLon)
    {
    ...
    public function mercatorDecrypt($mercatorLat, $mercatorLon)
    {
